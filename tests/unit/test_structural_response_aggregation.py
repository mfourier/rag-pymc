...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("answer_contract_evaluated_count", 6),
        ("output_contract_valid_given_valid_answer_contract_rate", 0.5),
        ("non_abstaining_zero_citation_response_count", 2),
        ("total_invalid_citation_count", 2),
        ("total_untraceable_citation_reference_count", 2),
    ],
)
def test_aggregate_metrics_reject_inconsistent_counts_and_rates(
    field_name: str,
    replacement: int | float,
) -> None:
    metrics = aggregate_structural_responses(make_mixed_responses()).metrics
    invalid = metrics.model_copy(update={field_name: replacement})

    with pytest.raises(ValidationError):
        AggregateStructuralResponseMetrics.model_validate(invalid)


def test_metrics_reject_more_invalid_citations_than_untraceable_references() -> None:
    values = aggregate_structural_responses(make_mixed_responses()).metrics.model_dump(
        mode="python"
    )
    values.update(
        total_valid_citation_count=1,
        total_invalid_citation_count=2,
        micro_citation_validity_rate=1 / 3,
    )

    with pytest.raises(ValidationError, match="every invalid citation"):
        AggregateStructuralResponseMetrics.model_validate(values)


def test_metrics_reject_invalid_citations_when_every_output_is_valid() -> None:
    values = aggregate_structural_responses(make_mixed_responses()).metrics.model_dump(
        mode="python"
    )
    values.update(
        output_contract_valid_count=4,
        output_contract_invalid_count=0,
        output_contract_valid_given_valid_answer_contract_rate=1.0,
        citation_traceability_valid_response_count=4,
        citation_traceability_invalid_response_count=0,
        citation_traceability_valid_given_valid_answer_contract_rate=1.0,
        structurally_valid_response_count=4,
        structurally_invalid_response_count=2,
        end_to_end_structural_validity_rate=4 / 6,
    )

    with pytest.raises(ValidationError, match="invalid citations and invalid outputs"):
        AggregateStructuralResponseMetrics.model_validate(values)


def test_metrics_reject_more_invalid_outputs_than_citation_bearing_responses() -> None:
    values = aggregate_structural_responses(make_mixed_responses()).metrics.model_dump(
        mode="python"
    )
    values.update(
        output_contract_valid_count=2,
        output_contract_invalid_count=2,
        output_contract_valid_given_valid_answer_contract_rate=0.5,
        citation_traceability_valid_response_count=2,
        citation_traceability_invalid_response_count=2,
        citation_traceability_valid_given_valid_answer_contract_rate=0.5,
        structurally_valid_response_count=2,
        structurally_invalid_response_count=4,
        end_to_end_structural_validity_rate=1 / 3,
        citation_bearing_response_count=1,
        zero_citation_response_count=3,
        non_abstaining_zero_citation_response_count=2,
        total_valid_citation_count=1,
        total_invalid_citation_count=2,
        micro_citation_validity_rate=1 / 3,
        total_traceable_citation_reference_count=1,
        total_untraceable_citation_reference_count=3,
        micro_citation_reference_traceability_rate=1 / 4,
    )

    with pytest.raises(ValidationError, match="invalid outputs cannot exceed"):
        AggregateStructuralResponseMetrics.model_validate(values)


def test_metrics_require_a_valid_citation_from_each_citation_bearing_valid_output() -> None:
    values = aggregate_structural_responses(make_mixed_responses()).metrics.model_dump(
        mode="python"
    )
    values.update(
        total_valid_citation_count=0,
        total_invalid_citation_count=3,
        micro_citation_validity_rate=0.0,
        total_traceable_citation_reference_count=0,
        total_untraceable_citation_reference_count=4,
        micro_citation_reference_traceability_rate=0.0,
    )

    with pytest.raises(ValidationError, match="citation-bearing valid output"):
        AggregateStructuralResponseMetrics.model_validate(values)


def test_metrics_reject_citations_without_citation_bearing_responses() -> None:
    valid_cited_response = make_mixed_responses()[0]
    values = aggregate_structural_responses((valid_cited_response,)).metrics.model_dump(
        mode="python"
    )
    values.update(
        citation_bearing_response_count=0,
        zero_citation_response_count=1,
        non_abstaining_zero_citation_response_count=1,
    )

    with pytest.raises(ValidationError, match="citations must exist exactly"):
        AggregateStructuralResponseMetrics.model_validate(values)


@pytest.mark.parametrize("reference_kind", ["traceable", "untraceable"])
def test_metrics_reject_more_unique_references_than_citation_claim_pairs(
    reference_kind: str,
) -> None:
    claim_ids = tuple(f"claim_{index}" for index in range(1, 11))
    valid_citation = make_valid_citation(
        "citation_valid",
        "chunk_a",
        context_position=1,
        referenced_claim_ids=("claim_1",),
    )
    if reference_kind == "traceable":
        invalid_citations = tuple(
            make_invalid_citation(
                f"citation_invalid_{index}",
                f"chunk_unknown_{index}",
            )
            for index in range(1, 10)
        )
        response = make_answer_record(
            "r_valid_reference_bound",
            is_abstaining=False,
            claim_ids=claim_ids,
            citation_results=(valid_citation, *invalid_citations),
        )
        count_updates: dict[str, int | float] = {
            "total_citation_reference_count": 20,
            "total_traceable_citation_reference_count": 11,
            "micro_citation_reference_traceability_rate": 11 / 20,
        }
    else:
        response = make_answer_record(
            "r_invalid_reference_bound",
            is_abstaining=False,
            claim_ids=claim_ids,
            citation_results=(valid_citation, make_invalid_citation()),
        )
        count_updates = {
            "total_citation_reference_count": 12,
            "total_untraceable_citation_reference_count": 11,
            "micro_citation_reference_traceability_rate": 1 / 12,
        }
    values = aggregate_structural_responses((response,)).metrics.model_dump(mode="python")
    values.update(count_updates)

    with pytest.raises(ValidationError, match=f"{reference_kind} references cannot exceed"):
        AggregateStructuralResponseMetrics.model_validate(values)


def test_metrics_reject_claims_in_an_all_abstention_population() -> None:
    abstention = make_answer_record("r_abstain", is_abstaining=True, claim_ids=())
    values = aggregate_structural_responses((abstention,)).metrics.model_dump(mode="python")
    values["total_claim_count"] = 1

    with pytest.raises(ValidationError, match="claims must exist exactly"):
        AggregateStructuralResponseMetrics.model_validate(values)


def test_metrics_reject_valid_citations_without_traceable_references() -> None:
    values = aggregate_structural_responses(make_mixed_responses()).metrics.model_dump(
        mode="python"
    )
    values.update(
    ...
    with pytest.raises(ValidationError, match="every valid citation"):
        AggregateStructuralResponseMetrics.model_validate(values)


def test_metrics_reject_references_to_claims_from_uncited_responses() -> None:
    cited = make_answer_record(
        "r_cited",
        is_abstaining=False,
        claim_ids=("claim_1",),
        citation_results=(
    ...
    uncited = make_answer_record(
        "r_uncited",
        is_abstaining=False,
        claim_ids=("claim_1",),
    )
    values = aggregate_structural_responses((cited, uncited)).metrics.model_dump(mode="python")
    values.update(
        total_citation_reference_count=2,
        total_traceable_citation_reference_count=2,
        micro_citation_reference_traceability_rate=1.0,
    )

    with pytest.raises(ValidationError, match="response allocation bound"):
        AggregateStructuralResponseMetrics.model_validate(values)
